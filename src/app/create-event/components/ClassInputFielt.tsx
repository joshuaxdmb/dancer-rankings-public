...
const ClassInputFielt = ({ editable, eventClass, setClass, index }: Props) => {
  return (
    <div className='bg-white bg-opacity-10 px-2 rounded-md py-1 mb-2'>
      ...
      <StyledTextInput
        className='pl-2 ml-1 mt-0'
        id={'instructors-' + index}
        value={eventClass.instructors}
        setValue={(value) => {
      ...
      <div className='flex flex-row items-center mt-0'>
      <p className='text-sm text-gray-200 mr-1'>Start Time:</p>
      <DateInputField birthdate={eventClass.start_time} setBirthdate={(value:any) => {
          setClass(index, { ...eventClass, start_time: value })
        }} disabled={!editable} />
      </div>
    </div>
  )

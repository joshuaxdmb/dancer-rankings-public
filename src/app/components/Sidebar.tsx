...
const Sidebar = ({ children }: Props) => {
  ...
  const sidebarContent = (
    <div className={`flex flex-col gap-y-2 h-full w-[300px] md:p-2 transition-transform duration-300 ${!visible ? '-translate-x-full' : ''}`}>
      <Box className="overflow-y-auto h-full">
        <button onClick={() => { setVisible(false) }} className="md:hidden p-6 pt-8 pb-0">
          <HiMiniXMark className="text-white" size={35} />
        </button>
        {routes.map((p) => (
          <Link key={p.href} href={p.href} className={twMerge('flex py-6 pl-6 text-xl flex-col hover:text-white transition cursor-pointer text-neutral-400', p.active && 'text-white')}>
            <h2 className="flex-row flex gap-x-1 items-center truncate w-full">
              {p.Icon ? <p.Icon size={22} /> : ''}{p.label}
            </h2>
          </Link>
        ))}
      </Box>
  ...
  return (
    <div className="flex h-full relative">
      ...
      <div className={`md:hidden fixed top-0 left-0 bottom-0 z-10 transition-transform duration-300 ${!visible ? '-translate-x-full' : ''}`}>
        {sidebarContent}
      </div>
      <main className={`h-full flex-1 overflow-y-auto py-2 ${visible? 'md:pl-[300px]':''}'md:pl-0' w-full`}>
        {children}
      </main>
    </div>
  );
